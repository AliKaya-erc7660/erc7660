...
console.log(ARBITRUM_URL,AVAX_URL);

//钱包provide
const provides = {
    arbitrumTest: new ethers.providers.JsonRpcProvider(ARBITRUM_URL),
    avax: new ethers.providers.JsonRpcProvider(AVAX_URL),
}

const signers = {
    arbitrum: ethers.Wallet(ARBITRUM_DEPLOY_KEY).connect(arbitrumTest),
    avax: ethers.Wallet(AVAX_DEPLOY_KEY).connect(avax),
}

const readCSV = async (file)=>{
    records = [];
    const parser = fs
        .createReadStream(file).pipe(parse({columns: true, delimiter: "," }));
