...
async function deployContractWithSigner(name, signer, args, label, options) {
  if (!options && typeof label === "object") {
    label = null;
    options = label;
  }
  ...
  //console.log("the contractFactory is ",contractFactory);
  let contract;
  if (options) {
    contract = await contractFactory.deploy(...args, options);
  } else {
...
function readTmpAddresses() {
  console.log("the process env NETWORK ",process.env.HARDHAT_NETWORK);
